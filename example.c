...
int main(int argc, char** argv) {
    ...
    struct font_t font_2;
    if(!leaf_load_font(&font_2, "Topaz-8.ttf")) {
        return 1;
    }

    font_2.center_char_to_cell = true;
    font_2.spacing = 0.5f;
    ...
    while(!glfwWindowShouldClose(leaf_ctx->glfw_win)) {
...

        {
            ...
            struct font_t* FN = &font_2;

            leaf_set_font_scale(FN, FS);
            leaf_set_font_scale(FN, FS);
            leaf_draw_text(FN, 50, 400, "Hello world! This is another font, |jlkmnio0++-_", -1);
            leaf_draw_text(FN, 50, 420, "R0=0", -1);
            leaf_draw_text(FN, 50, 420+20, "R1=0", -1);

            int text_w;
            int text_h;
            leaf_measure_text(FN, &text_w, &text_h, "R1=0", -1);

            leaf_draw_rect(50, 420+40, text_w, text_h, (struct color_t){ 30, 120, 50 });

            leaf_draw_text(FN, 50, 420+40, "R2=0", -1);
